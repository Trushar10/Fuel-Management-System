...
export async function POST(request) {
  try {
    ...
    const result = await db.execute({
      sql: `INSERT INTO fuel_entries (date, vehicle_no, driver_phone, driver_name, start_km, fuel_qty, end_km, filling_place, fuel_rate, total_fuel_cost)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
// DELETE /api/fuel-entries (delete all)
export async function DELETE() {
  try {
    const db = await getDb();
    await db.execute('DELETE FROM fuel_entries');
