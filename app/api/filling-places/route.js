...
export async function POST(request) {
  try {
    ...
    const result = await db.execute({ sql: 'INSERT INTO filling_places (name, is_mru, associate_vehicle) VALUES (?, ?, ?)', args: [name.trim(), is_mru ? 1 : 0, (associate_vehicle || '').trim()] });
...
export async function DELETE() {
  try {
    const db = await getDb();
    await db.execute('DELETE FROM filling_places');
